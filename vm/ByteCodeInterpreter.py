...
class ByteCodeInterpreter:
    ...
    def interpret(method: Method, log_inst: bool, args):
        ...
        j_args = ByteCodeInterpreter.create_args_array(method.get_class().loader, args)
        frame.local_vars.set_ref(0, j_args)

        try:
            ByteCodeInterpreter.loop(thread, log_inst)
        except RuntimeError as e:
            ByteCodeInterpreter.log_frames(thread)
    ...
    def loop(thread, log_inst):
        ...
        while True:
            ...
            if log_inst:
                ByteCodeInterpreter.log_instruction(frame, inst)
    ...
    def log_instruction(frame, inst):
        ...
        print("{0}.{1}() #{2:<2} {3} {4} operand_stack: {5} local_vars: {6}".format(class_name, method_name, pc,
                                                                                    ...
                                                                                    ByteCodeInterpreter.print_obj(inst),
                                                                                    frame.operand_stack,
                                                                                    frame.local_vars))
